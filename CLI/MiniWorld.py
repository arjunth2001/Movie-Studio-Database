...
######## Settings should be updated before running, instructions are in README , Comments in the global Scope can also be followed
######## The code is self-explanatory with variable names, and function names.
######## Show functions have formatted output :) 

def displayActor():
    
    try:

        cur.execute('select * , TIMESTAMPDIFF(YEAR, DOB, CURDATE()) AS Age from Actor') # Age is Derived Data here, Derived from DOB 
        con.commit()
        rows = cur.fetchall()
        wid1=len("Aadhar_ID")
        wid2=len("Name")
        ...
        for row in rows:
            ...
            wid8=max(wid8,len(str(row['Age'])))
        print(f"{'Aadhar_ID':<{wid1}s} : {'Name':<{wid2}s} : {'DOB':<{wid3}s} :  {'Age':<{wid8}s} : {'Contact':<{wid4}s} : {'Remuneration':<{wid5}s} : {'Lead or Not':<{wid6}s} : {'Filmography':<{wid7}s}")
        
        
        for row in rows:
            print(f"{str(row['Aadhar_ID']):<{wid1}s} : {str(row['Name']):<{wid2}s} : {str(row['DOB']):<{wid3}s} : {str(row['Age']):<{wid3}s} : {str(row['Contact']):<{wid4}s} : {str(row['Remuneration']):<{wid5}s} : {str(row['Leadflag']):<{wid6}s} : {str(row['Filmography']):<{wid7}s}")
...
def displayDirector():
    ...
    try:
        cur.execute('select * , TIMESTAMPDIFF(YEAR, DOB, CURDATE()) AS Age from Director') # Age is Derived Data here, Derived from DOB 
        con.commit()
        rows = cur.fetchall()
        ...
        for row in rows:
            ...
            wid7=max(wid7,len(str(row['Age'])))
            
        print(f"{'Director_ID':<{wid1}s} : {'Name':<{wid2}s} : {'DOB':<{wid3}s} : {'Age':<{wid7}s} : {'Filmography':<{wid4}s} : {'Mainflag':<{wid5}s} : {'MainID':<{wid6}s}")
        
        for row in rows:
            print(f"{str(row['Director_ID']):<{wid1}s} : {str(row['Name']):<{wid2}s} : {str(row['DOB']):<{wid3}s} : {str(row['Age']):<{wid7}s} : {str(row['Filmography']):<{wid4}s} : {str(row['Mainflag']):<{wid5}s} : {str(row['MainID']):<{wid6}s}")
...
def numSuccDirectors():
    ...
    try:  
        ...
        for row in rows:
            if row['Profit_or_Loss'] is not None and row['Profit_or_Loss'] >= avgProfit :
...
def valuedActors():
    ...
    try:
        ...
        for row in rows:
            awds = 1
            awds += row['count']
            cur.execute(f'select COUNT(DISTINCT(Movie_ID)) as acted from Actor natural join REQUIRES where Aadhar_ID = {row["Aadhar_ID"]}')
            con.commit()
            actVal = cur.fetchall()
            actedIn = actVal[0]['acted']
            sumt= sumt + awds * actedIn
            tot=tot+1
        if(tot==0):
            sumt=0
        else:
            sumt=sumt/tot
        print(f'Current Average experience value of Actors relative to studio present in Database is {sumt}') 
        
    except Exception as e:
        con.rollback()
        print("Failed to get Average experience value relative to studio. Try to continue by giving input")
        print(">>>>>>>>>>>>>", e)
    try:
        thresh = int(input("Enter the threshold experience value actors should possess relative to studio:  "))
        cur.execute(f'select * from Actor as A, (select Aadhar_ID, COUNT(*) as count from Actor_Accolades group by Aadhar_ID) as B where A.Aadhar_ID = B.Aadhar_ID')
        con.commit()
        rows = cur.fetchall()
        req = 0
        ...
        for row in rows:
            ...
            if ((awds * actedIn) >= thresh):
                req += 1
        print(f'Number of actors with / more than given threshold experience value relative to studio is {req}') 
        
    except Exception as e:
        con.rollback()
        print("Failed to get actors having more than given threshold experience value relative to studio")
        print(">>>>>>>>>>>>>", e)
    return
...
def insertmovie():
    ...
    try:
        ...
        if(Collections!=""):
            Collections=int(Collections)
            Budget=int(Budget)
            profit=Collections-Budget
            Movie_Collectionsquery = f'insert into Movie_Profit_or_Loss VALUES ({Movie_ID}, {profit})'  # Here Profit is derived from Collections and Budget 
            cur.execute(Movie_Collectionsquery)
            con.commit()
        else:
            Movie_Collectionsquery = f'insert into Movie_Profit_or_Loss (Movie_ID) VALUES ({Movie_ID})'
            cur.execute(Movie_Collectionsquery)
            con.commit()
...
def insertactor():
    try:
        ...
        Actor_Contact = int(input("Actors Contact number : "))
        if (Actor_Contact < 0):
        ...
        if (Actor_Remuneration < 0):
            ...
            return
        Actor_Leadflag = int(input("enter 1 if he is main actor,otherwise 0 : "))
...
def insertActorAddress():
    ...
    try:
        ...
        Actor_Address_Countryquery = f'insert into Actor_Address_Country VALUES ({Actor_Aadharid},"{Actor_Address_City}", "{Actor_Address_Country}")'
...
def insertDirectorAddress():
    ...
    try:
        ...
        Director_Address_Countryquery = f'insert into Director_Address_Country VALUES ({Directorid},"{Director_Address_City}", "{Director_Address_Country}")'
...
def insertVFXAddress():
    ...
    try:
        ...
        VFX_Address_Countryquery = f'insert into VFX_Address_Country VALUES ({VFXid},"{VFX_Address_City}", "{VFX_Address_Country}")'
...
def insertset():
    ...
    try:
        ...
        if (Man_Power < 0):
            ...
            return
        cur.execute(f'insert into Set_Manpower VALUES ({Movie_ID}, {Man_Power},{VFX_ID}, "{Location_Depicted}")')
        con.commit()
    except Exception as e:
        con.rollback()
        print(">>>>>>>>>>>>>", e)
    ...
    try:
        Set_Cost = int(input("Cost for the creation of set : "))
        if (Set_Cost < 0):
            print("Cost cannot take negative values")
            return
        cur.execute(f'insert into Set_Cost VALUES ({Movie_ID}, {Set_Cost},{VFX_ID}, "{Location_Depicted}")')
        con.commit()
...
def insertCOORDINATES_WITH():
    try:
        ...
        Director_ID = int(input("Director ID: "))
        cur.execute(f'insert into COORDINATES_WITH VALUES ({Director_ID}, {Movie_ID})')
        con.commit()
    except Exception as e:
        con.rollback()
        print(">>>>>>>>>>>>>", e)
...
def insertREQUIRES():
    ...
    for ind in range(noofsets):
        Set_ID = input(f'Enter Location_Depicted of Set {ind+1} : ')
        Sets.append(Set_ID)
    for x in Actors:
        for y in Sets:
            try:
                cur.execute(f'insert into REQUIRES VALUES ({x}, {Director_ID}, {Movie_ID}, "{y}")')
                con.commit()
            except Exception as e:
                con.rollback()
                print(">>>>>>>>>>>>>", e)
...
def updatemovie():
    ...
    try:
        ...
        if(op==1):
            ...
            con.commit()
            cur.execute(f'select (Collections-Budget) as profit from Movie_info natural join Movie_Collections where Movie_ID={Movie_ID}')
            con.commit()
            ans=cur.fetchone()
            ans=ans['profit']
            if(ans is None):
                return
            cur.execute(f'Update Movie_Profit_or_Loss set Profit_or_Loss={ans} where Movie_ID={Movie_ID}')
            con.commit()
            ...
            if(newvalue<0):
                print("Invalid")
                return
            cur.execute(f'Update Movie_Collections set Collections ={newvalue} where Movie_ID={Movie_ID}')
            con.commit()
            cur.execute(f'select (Collections-Budget) as profit from Movie_info natural join Movie_Collections where Movie_ID={Movie_ID}')
            con.commit()
            ans=cur.fetchone()
...
def updateactor():
    ...
    try:
        ...
        Aadhar_ID = int(input("Aadhar_ID of actor that need to updated: "))
        if (Aadhar_ID < 0):
...
def updatedirector():
    ...
    try:
        ...
        Director_ID = int(input("Director_ID of director that need to updated: "))
        if (Director_ID < 0):
            print("Director_ID cannot take negative values")
            return
        print("What do you want to Update?")
...
def updateSET():
    ...
    try:
        ...
        Movie_ID = int(input("Movie_ID of set that need to updated: "))
        ...
        if(op==1):
            ...
            cur.execute(f'Update Set_Location set Location_Depicted ="{newvalue}" where Movie_ID={Movie_ID} and Location_Depicted= "{oldlocation}"')
            con.commit()
...
def updateVFX():
    ...
    try:
        ...
        Movie_ID = int(input("Movie_ID of VFX studio that need to updated: "))
        if (Movie_ID < 0):
            print("Movie_ID cannot take negative values")
            return
        print("What do you want to Update?")
...
def updateREQUIRES():
    try:
        ...
        Set_IDold = input("old Location_Depicted : ")
        Director_IDold = int(input("Old director ID : "))
        Aadhar_IDnew = int(input("new Aadhar ID : "))
        Movie_IDnew = int(input("new VFX ID : "))
        Set_IDnew = input("new Location_Depicted : ")
        Director_IDnew = int(input("new director ID : "))
        cur.execute(f'update REQUIRES SET Aadhar_ID = {Aadhar_IDnew}, Director_ID = {Director_IDnew}, Movie_ID = {Movie_IDnew}, Location_Depicted = "{Set_IDnew}" where Aadhar_ID = {Aadhar_IDold} and Director_ID = {Director_IDold} and Movie_ID = {Movie_IDold} and Location_Depicted = "{Set_IDold}"')
        con.commit()
...
def REQUIRES_del():
    try:
        ...
        Set= input("Enter the Location_Depicted of row to delete :  ")
        cur.execute(f'delete from REQUIRES where Aadhar_ID={Aad} and Director_ID={Dir} and Movie_ID={Mov} and Location_Depicted="{Set}"')
        con.commit()
...
def Actor_Add_del():
    try:
        ...
        cur.execute(f'delete from Actor_Address_Country where Aadhar_ID={Aadhar_ID} and City="{City}"')
        con.commit()
...
def Director_Add_del():
    try:
        ...
        City = input("Enter the City of the Address to delete: ")
        cur.execute(f'delete from Director_Address_Country where Director_ID={Director_ID} and City="{City}"')
        con.commit()
...
def VFX_Add_del():
    ...
    try:
        ...
        City = input("Enter the City of the Address to delete: ")
        cur.execute(f'delete from VFX_Address_Country where Movie_ID={Movie_ID} and City="{City}"')
        con.commit()
        cur.execute(f'delete from VFX_Address_State where Movie_ID={Movie_ID} and City="{City}"')
        con.commit()
        cur.execute(f'delete from VFX_Address_City where Movie_ID={Movie_ID} and City="{City}"')
        con.commit()
...
def showREQUIRES():
    ...
    try:
        ...
        print(f"{'Movie_ID':<{wid1}s} : {'Aadhar_ID':<{wid2}s} : {'Director_ID':<{wid3}s} : {'Location_Depicted':<{wid4}s} ")
        
        for row in rows:
            print(f"{str(row['Movie_ID']):<{wid1}s} : {str(row['Aadhar_ID']):<{wid2}s} : {str(row['Director_ID']):<{wid3}s} : {str(row['Location_Depicted']):<{wid4}s}")
...
# Global - Set the settings Correctly
while(1):
    ...
    try:
        ...
        con = pymysql.connect(host='localhost',
                              user=username,
                              port=5005, # this was added for Docker Setup, remove if not required , or change to the required port 
                              password=password,
                              db='MOVIE_STUDIO', # the database name is MOVIE_STUDIO
        ...
        with con.cursor() as cur:
            while(1):
                ...
                print("8. Number of actors above given threshold experience value relative to studio")
